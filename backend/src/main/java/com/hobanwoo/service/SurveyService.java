...
public class SurveyService {
    ...
    public SurveyResultResponse calculateResult(SurveyResult surveyResult) {
        // 1. 점수판 준비 (동일)
        ...
        for (SurveyResult.AnswerDto answer : surveyResult.getAnswers()) {
            Question question = questionRepository.findById(answer.getQuestionId())
                    .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 질문입니다."));

            // 🌟 수정된 부분 1: String이 아니라 Enum 타입으로 가져옵니다.
            QuestionType type = question.getCategory();
            String pick = answer.getPick();

            // 🌟 수정된 부분 2: switch문에서 따옴표("")를 빼고 Enum 상수 그대로 적습니다.
            switch (type) {
                case EI:
                    if ("A".equals(pick)) eScore++; else iScore++;
                    break;
                case SN:
                    if ("A".equals(pick)) sScore++; else nScore++;
                    break;
                case TF:
                    if ("A".equals(pick)) tScore++; else fScore++;
                    break;
                case JP:
                    if ("A".equals(pick)) jScore++; else pScore++;
                    break;
            }
        }

        // 3. 최종 MBTI 알파벳 조합하기 (동일)
